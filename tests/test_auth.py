...
def test_auth_main_device_code_failure():
    ...
    with (
        patch.dict("os.environ", {"TRAKT_CLIENT_ID": "cid", "TRAKT_CLIENT_SECRET": "csecret"}),
...
def test_auth_main_success(capsys):
    ...
    token_resp.json.return_value = {"access_token": "acc_tok", "refresh_token": "ref_tok"}
...
def test_auth_main_refresh_flag_success(capsys):
    ...
    with (
        ...
        patch("sys.argv", ["auth.py", "--refresh"]),
...
def test_auth_main_refresh_missing_refresh_token():
    with (
        patch.dict(
            "os.environ", {"TRAKT_CLIENT_ID": "cid", "TRAKT_CLIENT_SECRET": "csecret"}, clear=True
        ),
        patch("builtins.input", return_value=""),
        pytest.raises(SystemExit) as exc_info,
    ):
...
def test_auth_main_refresh_failure():
    with (
        ...
        patch("auth.refresh_oauth_token", return_value=None),
        pytest.raises(SystemExit) as exc_info,
    ):
        auth.main(["--refresh"])
    assert exc_info.value.code == 1
